...
<article class="py-4 border-b border-grey-lightest">
	...
	@if(get_the_terms(get_the_id(), 'category'))
		@foreach(get_the_terms(get_the_id(), 'category') as $key => $term)
			<a href="{{ get_post_type_archive_link(get_post_type()) }}?{{'filter[category]=' .  $term->slug }}" class="px-2 mr-2 mb-2 py-1 text-sm no-underline hover:underline focus:underline text-black bg-grey-lightest rounded-full inline-block">{{ $term->name }}</a>
		@endforeach
	@endif
</article>
