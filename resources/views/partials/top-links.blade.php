		...
		@foreach($top_links as $key => $top_link)
			@if($top_link["external_link_toggle"])
				<li class="px-4 py-4 border-grey-lightest truncate overflow-hidden relative">
					...
					<a class="text-black whitespace-no-wrap hover:opacity-75 hover:underline" href="{{ $top_link["external_link"]["link"] }}">{{ $top_link["external_link"]["link_label"] ? $top_link["external_link"]["link_label"] : $top_link["external_link"]["link"]}}</a>
			...
			@else
				<li class="px-4 py-4 truncate overflow-hidden relative">
					<div class="absolute bg-yellow pin-l pin-t h-full w-1"></div>
					<a class="text-black whitespace-no-wrap hover:opacity-75 hover:underline" href="{{ get_permalink($top_link["internal_link"]["link"][0]->ID) }}">{{ $top_link["internal_link"]["link_label"] ? $top_link["internal_link"]["link_label"] : $top_link["internal_link"]["link"][0]->post_title }}</a>
				</li>
			@endif
		@endforeach
	</ol>
