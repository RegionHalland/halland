...
<article class="px-6 py-6 bg-green-light rounded">
    @if(isset($link) && !empty($link))
        <a class="mt-2 inline-block text-black font-sans hover:opacity-75" href="{{ $link['url'] }}">
    ...
    @if(isset($link) && !empty($link))
        <a target="{{ $link['target'] }}" class="underline hover:opacity-75 text-black font-sans text-lg" href="{{ $link['url'] }}">{{ $link['title'] }}</a>
   	@endif
</article>